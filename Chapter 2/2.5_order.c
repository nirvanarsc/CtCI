...
node convertToList(int sum) {
  int *arr = toArray(sum);
  return fromReverseArray(arr, sizeof(*arr));
}
...
int main() {
  ...
  int sum = convertFromReverseList(head) + convertFromReverseList(head2);
  printf("The sum of %d and %d is %d\n", convertFromReverseList(head),
         convertFromReverseList(head2), sum);
